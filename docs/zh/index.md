...
已有进行中的目标时，发送 `/goal` 后需要先用按钮确认更换。确认后，发送新的起始照片；可以 @机器人，也可以直接回复机器人的提示消息。体重可以写在配文中，也可以等机器人询问后单独回复。只有新目标创建完成后，旧目标才会进入历史记录。
...
- 最终目标体重和日期始终保持精确。
- 每位用户每分钟最多生成一次图片。
- 机器人会在周四提醒本周目标；如果整周还没有打卡，周日 10:00 会再次提醒。
- 完成每周目标可依次解锁水豚成就。
